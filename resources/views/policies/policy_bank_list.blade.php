@extends('layouts.app', ['page_title' => 'PolicyBank'])
@section('titles', 'Banks')
@section('title', 'policy Bank')
@section('content')
                ...
                <div class="card card-custom">
                    <div class="card-header">
                        <div class="card-title">
                            ...
                            <h3 class="card-label">Policy - {{ $policy->name }}</h3>
                        </div>
    ...
    <div class="modal fade" id="addItemModel" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
                <!--begin::Form-->
                <form id="kt_modal_add" class="form" method="POST" enctype="multipart/form-data" action="{{ route('policy.bank.store') }}">
                    @csrf
                    ...
                    <div class="modal-body">
                        <div class="form-group row">
                            ...
                            <div class="col-lg-12">
                                <div class="form-group">
                                    <label>@yield('title') Name:</label>
                                    <input type="text" id="item_name" class="form-control" name="name"  disabled/>
                                    
                                </div>
                                <div class="form-group">
                                    <label>@yield('title') Description:</label>
                                    <textarea class="form-control" id="item_desc" class="form-control" name="name"  disabled  rows="6"></textarea>
...
@push('scripts')
    <script>
        $("#kt_datatable").on('click', '.view-item', function () {
            ...
            const method = 'GET';
            const url = "{{route('policy.view.bank.list')}}"
            const data = {itemID};
            $.ajax({ method, url, data, dataType: 'json',
                success: function (data) {
                    console.log(data)
                    $("#item_name").val(data.bank);
        ...
        var KTDatatablesSearchOptionsAdvancedSearch = function() {
			...
			<'row'<'col-sm-12 col-md-5'i><'col-sm-12 col-md-7 dataTables_pager'lp>>`,
                    ...
                    serverSide: true,
                    ajax: {
                        url: "{{ route('policy.ajax.bank.list') }}",
                        ...
                        data: {
                            ...
                            itemID,
                            columnsDef: [
                                'DT_RowIndex', 'id', 'bank_id',  'created_at', 'updated_at', 'Actions'],
                        },
                    },
                    columns: [
