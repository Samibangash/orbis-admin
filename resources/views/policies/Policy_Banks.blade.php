...
@section('content')
    ...
    <div class="modal fade" id="editItemModel" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
                <!--begin::Form-->
                <form id="kt_modal_edit" class="form" method="POST" enctype="multipart/form-data" action="{{ route('policy.bank.update') }}">
                    @csrf
                    @method('PUT')
                    <div class="modal-header">
...
@push('scripts')
    <script>
        ...
        $("#kt_datatable").on('click', '.edit-item', function () {
            ...
            const method = 'POST';
            const url = "{{ route('policy.bank.edit') }}";
            ...
            $.ajax({ method, url, headers, data, dataType: 'json',
                success: function (data) {
                    ...
                    (data.image_path !== null)
                        ? $("#item_image_placeholder").css('background-image','url({{ URL::asset('/storage/images/policy_bank')}}/'+data.image_path+')')
        ...
        var KTDatatablesSearchOptionsAdvancedSearch = function() {
			...
			<'row'<'col-sm-12 col-md-5'i><'col-sm-12 col-md-7 dataTables_pager'lp>>`,
                    ...
                    serverSide: true,
                    ajax: {
                        url: "{{ route('policy.bank.ajax.list') }}",
                    ...
                    columnDefs: [
                        {
                            ...
                            render: function(data, type, full, meta) {
                                if(data !== null) {
                                    return '<img class="max-w-40px" src="{{ asset('/storage/images/policy_bank') }}/'+ data +'">';
                                } else {
